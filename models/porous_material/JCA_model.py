...
############### MODELO JCA - Jhonson-Champoux-Allard- ###############
...
# Parâmetros macroscópicos do material poroso (lã de rocha)
rf = 25743                         # resistividade ao fluxo [Ns/m^4]
por = 0.92                         # porosidade [%]
tor = 1.0                          # tortuosidade [-]
ccv = 42e-6                        # comprimento característico viscoso [m]
cct = 142e-6                       # comprimento característico térmico [m]
rho2 = 64                          # densidade do material fibroso [Kg/m^3]
...
for o in range(len(f)):
    ...
    keq[o] = ((gamma * Po) / por) / (gamma - (gamma - 1) * (1 + (8 * k_f) / (1j * w[o] * C_p * cct**2 * po) * np.sqrt(1 + (1j * w[o] * C_p * po * cct**2) / (16 * k_f)))**-1)
...
# Plotagem
plt.figure()
plt.plot(f, alphar, 'r', linewidth=3)
plt.grid(True)
plt.xlim([100, 10000])
plt.xlabel('Frequência - [Hz]')
plt.ylabel(r'$\alpha(\omega)$ [-]')
plt.legend(['JCA - model'])
plt.show()