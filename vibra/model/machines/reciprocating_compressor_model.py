...
class ReciprocatingCompressorModel:
    ...
    tdc_crank_angle : float = 0.                                            # Crank angle (degrees) at which piston in the head-end chamber is at top dead center
    ...
    @property
    def tdc(self):
        return self.tdc_crank_angle * pi / 180
    ...
    def process_head_end_volumes_and_pressures(
        ...
        if export_data:
            ...
            folder_path = VIBRA_DIR.cwd() / "temporary_data"
            folder_path.mkdir(exist_ok=True)

            fname = folder_path / f"PV_diagram_head_end_crank_angle_{self.tdc_crank_angle}.dat"
            fname_log =folder_path / f"log_info_head_end_{self.tdc_crank_angle}_cap_{capacity}.txt"
            ...
            indices = np.arange(N)
            data = np.array([indices, time, angle, v_piston, volumes, pressures, open_suc, open_disc], dtype=float)
    ...
    def process_crank_end_volumes_and_pressures(
        ...
        if export_data:
            ...
            fname = folder_path / f"PV_diagram_crank_end_crank_angle_{self.tdc_crank_angle}.dat"
            fname_log =folder_path / f"log_info_crank_end_{self.tdc_crank_angle}_cap_{capacity}.txt"
    ...
    def get_cycle_indices(self, start_index: int, end_index: int, N: int):
