...
class ProjectFile:
    ...
    def _default_paths(self):
        ...
        self.solution_freq_filepath = self.path / "solution_freq.hdf5"
                ...
                if analysis:
                    ...
                    if frequencies:
                        ...
                        if solution_dset.chunks is None:
                            ...
                            writer = LazyHDF5MatrixWriter(self.solution_freq_filepath, solution.shape[0], frequencies, solution.dtype)
                            for i, freq in enumerate(frequencies):
                                writer[:, i] = solution[:, i]

    def get_solution_writer(self, num_rows, columns, dtype):
        return LazyHDF5MatrixWriter(self.solution_freq_filepath, num_rows, columns, dtype)

    def get_solution_loader(self):
        return LazyHDF5MatrixLoader(self.solution_freq_filepath)

    def delete_harmonic_solution(self):
        if self.solution_freq_filepath.exists():
            os.remove(self.solution_freq_filepath)
