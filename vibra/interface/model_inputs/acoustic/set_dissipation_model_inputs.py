...
class DissipationModelInput(QDialog):
    ...
    def _define_qt_variables(self):
        ...
        self.tab_proportional_damping = self.tabWidget_dissipation_model.findChild(QWidget, "tab_proportional_damping")
    ...
    def check_dissipation_model_entries(self):
        ...
        if tab_index == 0:
            ...
            self.speed_of_sound_factor = self.check_inputs( lineEdit, 
                                                            "Speed of sound complex factor", 
                                                            only_positive = True )
            if self.stop:
                lineEdit.setFocus()
                return True
            ...
            self.fluid_density_factor = self.check_inputs(  lineEdit, 
                                                            "Fluid density complex factor",
                                                            only_positive = True  )
            if self.stop:
                lineEdit.setFocus()
                return True
