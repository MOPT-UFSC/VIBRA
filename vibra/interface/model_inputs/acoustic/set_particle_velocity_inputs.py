...
class ParticleVelocityInput(QDialog):
    ...
    def _create_connections(self):
        ...
        self.treeWidget_particle_velocity.itemDoubleClicked.connect(self.on_doubleclick_item)
    ...
    def check_complex_entries(self, lineEdit_real, lineEdit_imag):
        ...
        if lineEdit_real.text() != "":
            try:
                ...
                message = "Wrong input for real part of particle velocity."
        ...
        if lineEdit_imag.text() != "":
            try:
                ...
                message = "Wrong input for imaginary part of particle velocity."
                PrintMessageInput([title, message, window_title_1])
    ...
    def save_table_file(self, entity_id, values, filename):
        try:
            ...
            header = f"Vibra - imported table for particle velocity @ surface {entity_id} \n"
    ...
    def get_list_table_names_from_selected_surfaces(self, list_ids):
        list_table_names = []
        for surface_id, data in self.properties.surfaces_with_particle_velocity.items():
    ...
    def remove_bc_from_selection(self):
        if self.lineEdit_selection_id.text() != "":
            ...
            if picked_id in self.properties.surfaces_with_particle_velocity.keys():
                section_key = f"surface - {picked_id}"
                key_strings = ["particle velocity", "averaged", "table name"]
                message = (
                    f"The particle velocity attributed to the {picked_id} surface has been removed."
                )
                self.project.file.remove_bc_from_file(
                    section_key, self.acoustic_bc_info_path, key_strings, message
                )
    ...
    def check_reset(self):
        if len(self.properties.surfaces_with_particle_velocity) > 0:
            ...
            entity_ids = list(self.properties.surfaces_with_particle_velocity.keys())
            ...
            sections = []
            if read._continue:
                surfaces_ids = self.properties.surfaces_with_particle_velocity.keys()
                for _id in surfaces_ids:
                    key_strings = ["particle velocity", "averaged", "table name"]
                    sections.append(f"surface - {_id}")
                    data = self.properties.surfaces_with_particle_velocity[_id]
                    if "table_name" in data.keys():
                ...
                self.project.file.remove_bc_from_file(
                    sections, self.acoustic_bc_info_path, key_strings, None
                )
    ...
    def update_tabs_visibility(self):
        if len(self.properties.surfaces_with_particle_velocity) == 0:
            self.tabWidget_particle_velocity.setCurrentWidget(self.tab_constant_values)
