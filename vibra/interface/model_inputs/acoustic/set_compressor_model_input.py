...
class CompressorModelInput(QDialog):
    ...
    def _define_qt_variables(self):
        ...
        # QLineEdit
        self.lineEdit_suction_id : QLineEdit
        self.lineEdit_discharge_id : QLineEdit
        ...
        self.current_lineEdit = self.lineEdit_suction_id
    ...
    def _create_connections(self):
        ...
        #
        self.clickable(self.lineEdit_suction_id).connect(self.lineEdit_1_clicked)
        self.clickable(self.lineEdit_discharge_id).connect(self.lineEdit_2_clicked)
        #
    ...
    def lineEdit_2_clicked(self):
        self.current_lineEdit = self.lineEdit_discharge_id
    ...
    def geometry_selection_callback(self):
        ...
        if surfaces:
            ...
            if len(surfaces) == 1:
                self.current_lineEdit.setText(str(surfaces[0]))
            elif len(surfaces) == 2:
                return
    ...
    #         temp_text_suction = self.lineEdit_suction_id.text()
    #         temp_text_discharge = self.lineEdit_discharge_id.text()
    #         self.lineEdit_suction_id.setText(temp_text_discharge)
    #         self.lineEdit_discharge_id.setText(temp_text_suction)   
    ...
    def update_compressor_to_pipeline_connections(self):
        ...
        self.lineEdit_suction_id.setDisabled(False)
        self.lineEdit_discharge_id.setDisabled(False)     
        index = self.comboBox_connection_setup.currentIndex()

        selected_ids = self.main_window.selected_geometry_surfaces

        if index == 1:
            self.current_lineEdit = self.lineEdit_suction_id
            self.lineEdit_discharge_id.setDisabled(True)
            self.lineEdit_discharge_id.setText("")
            if len(selected_ids) == 1:
                self.lineEdit_suction_id.setText(str(selected_ids[-1]))

        elif index == 2:
            self.current_lineEdit = self.lineEdit_discharge_id
            self.lineEdit_suction_id.setDisabled(True)
            self.lineEdit_suction_id.setText("")
            if len(selected_ids) == 1:
                self.lineEdit_discharge_id.setText(str(selected_ids[-1]))
        else:
            self.current_lineEdit = self.lineEdit_suction_id
            self.update()
            
    # def writeNodes(self, selected_ids):
    ...
    #     self.current_lineEdit.setText(str(selected_ids[-1]))
    #     self.table_name = None
    #     self.comboBox_compressors_tables.clear()
    #     self.comboBox_compressors_tables.setVisible(False)
    #     self.get_existing_compressor_info(selected_ids, update_tables=True)
    ...
    def update_compressor_inputs(self, compressor_info):
        ...
        if 'discharge' in self.table_name:
            ...
            self.lineEdit_suction_id.setText("")
            self.lineEdit_suction_id.setDisabled(True)
            self.lineEdit_discharge_id.setText(node_ID)
            
        if 'suction' in self.table_name:
            self.comboBox_connection_setup.setCurrentIndex(1)
            self.lineEdit_discharge_id.setText("")
            self.lineEdit_discharge_id.setDisabled(True)
            self.lineEdit_suction_id.setText(node_ID)
    ...
    def check_surfaces_selection(self):

        index = self.comboBox_connection_setup.currentIndex()

        if index == 0:
            
            lineEdit = self.lineEdit_suction_id
            stop, self.suction_surface_id = self.mesh.check_selected_ids(lineEdit.text(), selection="surfaces", single_id=True)
            if stop:
                lineEdit.setFocus()
                return True

            lineEdit = self.lineEdit_discharge_id
            stop, self.discharge_surface_id = self.mesh.check_selected_ids(lineEdit.text(), selection="surfaces", single_id=True)
            if stop:
                lineEdit.setFocus()
                return True

            if self.suction_surface_id == self.discharge_surface_id:
    ...
    def process_all_inputs(self):
        ...
        if self.check_surfaces_selection():
            return
        ...
        if index in [0, 1]:
            ...
            volume_id = self.mesh.volume_from_surface[self.suction_surface_id]
            if len(volume_id) == 1:

                compressor_info = { "temperature (suction)" : self.T_suction,
                                    "pressure (suction)" : self.P_suction,
                                    "volume_id" : volume_id[0],
                                    "node_id" : self.suction_surface_id,
                ...
                if not read.complete:
                    ...
                    table_name = self.get_table_name(self.suction_surface_id, 'suction')
                    data = [in_flow_rate, table_name]

                self.project.remove_acoustic_pressure_table_files(self.suction_surface_id)
                self.project.remove_volume_velocity_table_files(self.suction_surface_id)

                if self.project.set_compressor_excitation_bc_by_node(   [self.suction_surface_id], 
                                                                        data, 
                                                                        self.table_index,
                                                                        'suction'   ):
                    return
        ...
        if index in [0, 2]:
            ...
            if len(volume_id) == 1:
                ...
                compressor_info = { "temperature (suction)" : self.T_suction,
                                    "pressure (suction)" : self.P_suction,
                                    "volume_id" : volume_id[0],
                                    "node_id" : self.discharge_surface_id,
                                    "pressure ratio" : self.parameters['pressure ratio'],
                                    "connection type" : 1 }
                ...
                table_name = self.get_table_name(self.discharge_surface_id, 'discharge')
                data = [out_flow_rate, table_name]

                self.project.remove_acoustic_pressure_table_files(self.discharge_surface_id)
                self.project.remove_volume_velocity_table_files(self.discharge_surface_id)

                if self.project.set_compressor_excitation_bc_by_node(   [self.discharge_surface_id], 
                                                                        data, 
                                                                        self.table_index, 
                                                                        'discharge'   ):
                    return
