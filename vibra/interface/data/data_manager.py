...
def is_frequencies_vector_equally_distributed(frequencies: list | np.ndarray):
    ...
    freq_increments = np.unique(np.array(shifted_freqs) - np.array(frequencies))

    return freq_increments.size == 1