...
class ACT_LINE_3(Element1D):
    ...
    def process_shape_functions_and_derivatives(self):
        ...
        pint_M, self.wps_M = self.get_integration_points_data(integration_points=2)
    ...
    def get_stacked_local_coordinates(self) -> np.ndarray:
        ...
        X3 = self.nodal_coordinates[self.connect_line[:, 2], 1]
        Y3 = self.nodal_coordinates[self.connect_line[:, 2], 2]
        Z3 = self.nodal_coordinates[self.connect_line[:, 2], 3]

        nel = self.connect_line.shape[0]
        vec_13 = np.array([X3-X1, Y3-Y1, Z3-Z1]).T
        vec_32 = np.array([X2-X3, Y2-Y3, Z2-Z3]).T

        coord_loc = np.zeros((nel, 3, 1), dtype=float)
    ...
    def reorder_connect(self, connect_line: np.ndarray):
        ...
        """
        self.connect_line = connect_line[:, [0, 1, 2]]
