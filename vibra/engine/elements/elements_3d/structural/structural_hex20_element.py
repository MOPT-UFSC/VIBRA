...
class STRUCT_HEXAHEDRON_20(Element3D):
    ...
    def stacked_elementary_matrices(self, material_properties: np.ndarray):
        ...
        for i in range(self.nint_K):
            ...
            progress = int(25 + 35 * (i / self.nint_K))
            ...
            stacked_jacs_K = self.dphi_K[i, :, :] @ stacked_coords

            # Jacobian determinants and inverses of all elements
            det_jacs_K, inv_jacs_K = self.get_detJAC_and_invJAC(stacked_jacs_K)

            # derivative of shape functions
            dphi_t = inv_jacs_K @ self.dphi_K[i, :, :]
            ...
            Ke += B.transpose((0, 2, 1)) @ D @ B * (det_jacs_K * self.wps_K[i])
        ...
        for i in range(self.nint_M):
            ...
            progress = int(60 + 35 * (i / self.nint_M))
            logging.info(f"Processing the elementary matrices data... [{progress}/100]")

            # Jacobian matrices of all elements of the i-th integration point
            stacked_jacs_M = self.dphi_M[i, :, :] @ stacked_coords

            # Jacobian determinants and inverses of all elements
            det_jacs_M, _ = self.get_detJAC_and_invJAC(stacked_jacs_M)
            ...
            Me += rho * N.T @ N * (det_jacs_M * self.wps_M[i])
