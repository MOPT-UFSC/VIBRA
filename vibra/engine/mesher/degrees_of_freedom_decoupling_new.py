...
class DegreesOfFreedomDecoupling:
        ...
        for key, data in self.properties.surface_properties.items():
            ...
            if property == "degrees_of_freedom_decoupling":
                ...
                if isinstance(vol_id, int):    
                    ...
                    self.volume_to_surfaces_map[vol_id].append(surface_id)
    ...
    def update_surface_property_with_new_surface_id(self):
        ...
        for surface_id in self.decouple_info:
    ...
    def get_new_points_mapping(self):
        ...
        new_point_ids = [int(i + max_point_id + 1) for i in range(len(point_ids))]
    ...
    def get_new_lines_mapping(self):
        ...
        new_line_ids = [int(i + max_line_id + 1) for i in range(len(line_ids))]
    ...
    def get_new_surfaces_mapping(self):
        ...
        new_surface_ids = [int(i + max_surface_id + 1) for i in range(len(surface_ids))]
    ...
    def get_surfaces_and_nodes_from_lines_that_bound_decoupling_surfaces(self):
        ...
        surfaces_to_remove = list(self.decouple_info.keys())
    ...
    def update_nodes_from_array(self, values: np.ndarray):
        ...
        """
        output_values = deepcopy(values)

        for j, node_id in enumerate(values):
            mapped_node_id = self.nodes_mapping.get(node_id)
            if mapped_node_id is None:
                continue

            output_values[j] = mapped_node_id
    ...
    def modify_the_connectivities_from_lines(self, surface_id: int):
        ...
        lines_from_surface = self.mesh.lines_from_surface.get(surface_id)
        ...
        for i, line_id in enumerate(lines_from_surface):
            ...
            last_index = int(self.mesh.lines_connectivity[-1, 0])
            element_ids = np.arange(rows, dtype=int) + last_index + 1
    ...
    def modify_the_connectivities_from_surfaces(self, surface_id: int):
        ...
        rows = new_connectivity.shape[0]
        ones = np.ones(rows, dtype=int)
        last_index = int(self.mesh.faces_connectivity[-1, 0])
        element_ids = np.arange(rows, dtype=int) + last_index + 1
    ...
    def get_lines_from_unmodified_surfaces(self):
        ...
        lines_nodes = set()
        for surface_id, vol_id in self.decouple_info.items():
            lines_from_surface = self.mesh.cache_lines_from_surface.get(surface_id)
            if lines_from_surface is None:
                continue

            for surf_id in self.mesh.cache_surfaces_from_volume.get(vol_id, []):
                if surf_id == surface_id:
                    continue
                
                line_ids |= set(self.mesh.cache_lines_from_surface.get(surf_id))
    ...
    def update_all_connectivity_related_attributes(self):
        ...
        for surface_to_decouple, volume_to_decouple in self.decouple_info.items():

            surface_nodes = self.mesh.get_nodes_from_surface(surface_to_decouple, from_cache=True)

            volume_rows = self.mesh.cache_solids_connectivity[:, 1] == volume_to_decouple
            volume_connectivities = self.mesh.cache_solids_connectivity[volume_rows, :]

            rows_3d = np.any(np.isin(volume_connectivities[:, 4:], surface_nodes), axis=1)

            for elem3d_id, vol_id, _, _, *connect_3d in volume_connectivities[rows_3d, :]:
                if vol_id != volume_to_decouple:
                    continue

                self.mesh.solids_connectivity[elem3d_id, 4:] = self.update_nodes_from_array(connect_3d)

        for volume, surfaces in self.volume_to_surfaces_map.items():
            neighbor_surfaces = set(self.mesh.cache_surfaces_from_volume.get(volume)) - set(surfaces)

            # nodes_per_element_2d = self.mesh.cache_faces_connectivity[0, 4:].size
            rows_f1 = np.isin(self.mesh.cache_faces_connectivity[:, 1], surfaces)
            surfaces_nodes = np.unique(self.mesh.cache_faces_connectivity[rows_f1, 4:].flatten())

            rows_2d = np.any(np.isin(self.mesh.cache_faces_connectivity[:, 4:], surfaces_nodes), axis=1)
            # rows_2d = np.isin(self.mesh.cache_faces_connectivity[rows_f2, :][:, 1], surfaces, invert=True)
            # surfaces_from_volume = self.mesh.cache_surfaces_from_volume.get(volume_to_decouple)

            for elem2d_id, surf_id, _, _, *connect_2d in self.mesh.cache_faces_connectivity[rows_2d, :]:
                if surf_id in neighbor_surfaces:
        ...
        # rows_2d = np.any(np.isin(self.mesh.cache_faces_connectivity[:, 4:], nodes_from_bound_lines), axis=1)
        # for elem2d_id, surf_id, _, _, *connect_2d in self.mesh.cache_faces_connectivity[rows_2d, :]:
        #     if surf_id in valid_surface_ids:
        #         self.mesh.faces_connectivity[elem2d_id, 4:] = self.update_nodes_from_array(connect_2d)

        # valid_line_ids, lines_nodes = self.get_lines_from_unmodified_surfaces()
        # print(len(valid_line_ids), len(lines_nodes))
        # rows_1d = np.any(np.isin(self.mesh.cache_lines_connectivity[:, 4:], lines_nodes), axis=1)
    ...
    def process_degrees_of_freedom_decoupling(self):
        ...
        self.mesh.process_disconnected_nodes_criterion()

        self.mesh.export_nodal_coordinates("nodal_coordinates.dat")
        self.mesh.export_solid_elements_connectivity("solid_connectivities.dat")
        self.mesh.export_face_elements_connectivity("face_connectivities.dat")
